    ...
    return (
      <div className="text-center py-8">
        ...
        <p className="text-sm text-gray-500 mt-1">
          Tambahkan perawatan untuk melihat riwayatnya di sini.
        </p>
      </div>
    );
  }
          ...
          <tbody className="divide-y divide-gray-200">
            {treatments
              ...
              .map((treatment) => (
                <tr
                  key={treatment.id}
                  className={`hover:bg-gray-50/70 transition-colors duration-150 ${isSubmitting ? 'opacity-70' : ''}`}
                >
                  ...
                  <td
                    className="px-5 py-4 text-sm text-gray-600 max-w-xs"
                  ...
                  <td className="px-5 py-4 whitespace-nowrap text-sm space-x-3">
                    <button
                      ...
                      aria-label={`Edit perawatan tanggal ${treatment.date ? new Date(treatment.date).toLocaleDateString() : '-'}`}
                    >
                      Edit
                    </button>
                    <button
                      ...
                      aria-label={`Hapus perawatan tanggal ${treatment.date ? new Date(treatment.date).toLocaleDateString() : '-'}`}
                    >
                      Hapus
                    </button>
                  </td>
